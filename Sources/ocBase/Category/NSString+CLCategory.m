...
- (NSString *)subStringWithPrefix:(NSString*)prefixStr PostfixStr:(NSString*)postfixStr
{
    if (prefixStr.length>0 && postfixStr.length>0){
        NSRange range1 = [self rangeOfString:prefixStr] ;
        NSRange range2 = [self rangeOfString:postfixStr] ;
        ...
        return nil ;
    }else if (prefixStr.length>0){
        NSRange range1 = [self rangeOfString:prefixStr] ;
        if (range1.location !=NSNotFound){
            return [self substringWithRange:NSMakeRange(range1.location+range1.length, self.length - range1.location - range1.length)] ;
        }
        
        return nil ;
    }else if (postfixStr.length>0){
        NSRange range2 = [self rangeOfString:postfixStr] ;
        if (range2.location !=NSNotFound){
            return [self substringWithRange:NSMakeRange(0, range2.location)] ;
        }
        return nil ;
    }
